...
class TempSceneController:
    def __init__(self, filename) -> None:
        ...
        self.MAKUAI_ID_FIRSTSESSION_INDEX = 2 # CSVで、Opening と Closing 等を除く最初のセッションが存在するインデックスを指定する(0始まり、ヘッダーは飛ばして数え始める)
        ...
        for i, session in enumerate(self.sessionsmodel.data):
            ...
            if i < self.MAKUAI_ID_INDEX_OFFSET: # １本目の登壇より CSV 的に上にあるシーン(Opening など)
                ...
                if session['title']=='Opening':
                    requests.append(simpleobsws.Request('CreateInput', {'sceneName': f"{name}", 'inputName':  f"{session['id']}_makuai_media", 'inputKind': 'vlc_source', 'inputSettings': {'playlist': [{'hidden': False, 'selected': False, 'value': '/home/ubuntu/Nextcloud/Broadcast/CNSec2022/Sync/Media/z-common/幕間.mp4'}]}}))
                    # Twitter ウィジェット
                    requests.append(simpleobsws.Request('CreateInput', {'sceneName': f"{name}", 'inputName':  f"{session['id']}_Twitter", 'inputKind': 'browser_source', 'inputSettings': {'height': 1080, 'is_local_file': True, 'local_file': '/home/ubuntu/Nextcloud/Broadcast/CNSec2022/Sync/Media/z-common/twitter_hashtag/index.html', 'width': 1920}}))
                    # アスペクト比フィルター
                ...
                ### Closing 後のエンドロール
                    ...
                    requests.append(simpleobsws.Request('CreateInput', {'sceneName': f"{name}", 'inputName':  f"{session['id']}_休憩_ended_image", 'inputKind': 'image_source', 'inputSettings': {'file': '/home/ubuntu/Nextcloud/Broadcast/CNSec2022/Sync/Media/z-common/本日のイベントは終了しました.png'}}))
                    # エンドロール向け VLC ソースを追加。カウントダウンは無し
                    # TODO: 格納確認
                    requests.append(simpleobsws.Request('CreateInput', {'sceneName': f"{name}", 'inputName':  f"{session['id']}_media", 'inputKind': 'vlc_source', 'inputSettings': {'loop': False, 'playlist': [{'hidden': False, 'selected': False, 'value': '/home/ubuntu/Nextcloud/Broadcast/CNSec2022/Sync/Media/z-common/EndCredit.mp4'}]}}))
                    # Twitter ウィジェット
                    requests.append(simpleobsws.Request('CreateInput', {'sceneName': f"{name}", 'inputName':  f"{session['id']}_Twitter", 'inputKind': 'browser_source', 'inputSettings': {'height': 1080, 'is_local_file': True, 'local_file': '/home/ubuntu/Nextcloud/Broadcast/CNSec2022/Sync/Media/z-common/twitter_hashtag/index.html', 'width': 1920}}))
                    # アスペクト比フィルター
            ...
            if session['title']=='Opening':
                ...
                requests.append(simpleobsws.Request('CreateInput', {'sceneName': f"{name}", 'inputName':  f"{session['id']}_media", 'inputKind': 'vlc_source', 'inputSettings': {'loop': False, 'playlist': [{'hidden': False, 'selected': False, 'value': '/home/ubuntu/Nextcloud/Broadcast/CNSec2022/Sync/Media/z-common/Opening.mp4'}]}}))
                # アスペクト比フィルター
                requests.append(simpleobsws.Request('CreateSourceFilter', {'sourceName': f"{session['id']}_media", 'filterName': 'スケーリング/アスペクト比', 'filterKind': 'scale_filter', 'filterSettings': {'resolution': '1920x1080'}}))
            ...
            ## 終了画像 with BGM
                ...
                requests.append(simpleobsws.Request('CreateInput', {'sceneName': f"{name}", 'inputName':  f"{session['id']}_ended_image", 'inputKind': 'image_source', 'inputSettings': {'file': '/home/ubuntu/Nextcloud/Broadcast/CNSec2022/Sync/Media/z-common/本日のイベントは終了しました.png'}}))
                requests.append(simpleobsws.Request('CreateInput', {'sceneName': f"{name}", 'inputName':  "main_bgm", 'inputKind': 'ffmpeg_source', 'inputSettings': {'local_file': '/home/ubuntu/Nextcloud/Broadcast/CNSec2022/Sync/Media/z-common/Possibilities - Josh Leake.mp3', 'looping': True}}))
